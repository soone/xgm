...
<div class="divform">
	...
	<!--{if $card}-->
	<!--{if ($card[6] == 3 || ($card[9] > 0 && $cType[6] == 1)) && $card[5] == 1}-->
	<h5>点击<a href="index.php?control=card&action=choose&clnum=<!--{$card[3]}-->&clid=<!--{$card[1]}-->&blance=<!--{$card[9]}-->">这里</a>开始挑选客户需要的物品</h5>
	<!--{/if}-->
	<table class="slist">
		<tr>
			<td><b>卡号</b></td>
			...
			<td><!--{$card[9]}--></td>
			<td><b>卡状态</b></td>
			<td><!--{if $card[5] == 1}-->可用<!--{else}-->不可用<!--{/if}--></td>
			<td><b>卡名称</b></td>
			<td><!--{$cType[1]}--></td>
			<td><b>卡类型</b></td>
			<td><!--{if $cType[6] == 1}-->储值卡<!--{else}-->储物卡<!--{/if}--></td>
		</tr>
		<tr>
			<td><b>有效期</b></td>
			<td><!--{$card[7]}--></td>
