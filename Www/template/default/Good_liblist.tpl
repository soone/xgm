...
<h3 class="topmenu">库存列表(<a href="index.php?control=good&action=getcart">查看购物车</a>)&nbsp;&nbsp;(总库存余量价值：<!--{$leaveMoney}-->)</h3>
<div>
<form>
	<label>选择分类：</label>
...
<table class="slist">
	...
	<tbody>
		...
		<tr>
			...
			<td><!--{$liblist[l].5}--></td>
			<td><!--{if $pInfo == 1}--><input type="text" class="text" style="width:30px" value="1" id="good<!--{$liblist[l].0}-->"/><a href="javascript:void(0);" onclick="addcart('<!--{$liblist[l].0}-->', '<!--{$liblist[l].1}-->', '<!--{$liblist[l].3}-->', '<!--{$liblist[l].4}-->', '<!--{$liblist[l].6}-->');">添加到购物车</a>&nbsp;|&nbsp;<!--{/if}--><a href="index.php?control=good&action=gedit&name=<!--{$liblist[l].1|escape:'url'}-->">查看详情</a>&nbsp;&nbsp;<a href="index.php?control=good&action=ginlist&name=<!--{$liblist[l].1|escape:'url'}-->">查看对应进货列表</a><!--{if $pInfo != 1}-->&nbsp;&nbsp;<a href="index.php?control=good&action=gindel&name=<!--{$liblist[l].1|escape:'url'}-->">删除</a><!--{/if}--></td>
		</tr>
		<!--{/section}-->
	</tbody>
</table>
